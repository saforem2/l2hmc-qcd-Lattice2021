...
%
\begin{wrapfigure}[19]{r}[36pt]{0.50\textwidth}
    \vspace{-\baselineskip}
    \centering
    \includegraphics[width=0.50\textwidth]{assets/qfreezing.pdf}
...
This freezing can be quantatively measured by introducing the \emph{tunneling rate}\footnote{As measured between subsequent states in our chain \(i\), \(i+1\).}
%
\begin{equation}
...
\begin{figure}[htpb]
    ...
    leapfrog update. In Fig.~\ref{subfig:updates},\ref{subfig:network_fns}, we
denote \(\bar{\mathbf{x}}_{k} = \maskbar\odot \mathbf{x}_{k}\).}
\end{figure}
%
...
\begin{wrapfigure}[14]{r}{0.55\textwidth}
    \vspace{-\baselineskip}
    \centering
    \includegraphics[width=0.54\textwidth]{assets/autocorr_vs_beta.pdf}
    \caption{\label{fig:autocorr}Estimated
    \(\tau_{\mathrm{int}}^{\mathcal{Q}_{\mathbb{R}}}\) vs \(\beta\).}
    %, scaled by \(N_{\mathrm{LF}}\) to account for simulation cost.}
\end{wrapfigure}
%
...
% \begin{subfigure}[t]{\textwidth}
    \includegraphics[width=0.9\textwidth]{assets/ridgeplots.pdf}
    \caption{\label{fig:ridgeplots}Illustration of how different quantities
    change during the generalized leapfrog update.}%
...
\begin{figure}[htpb]
    \centering
    \includegraphics[width=\textwidth]{assets/plaqsf_vs_lf_step_1659.pdf}
    \caption{\label{fig:plaqsf_beta}Plots showing the deviation in average
    plaquette vs leapfrog step from the expected value in the infinite
volume limit}
\end{figure}
%
