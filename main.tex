...
however, they are not generally applicable to full QCD.
%
The method presented here can be extended to full QCD in a relatively
straightforward manner, though the cost of training and final
effectiveness are open questions.
%
For this reason we only consider HMC as a baseline general method to
compare performance to.
%
\subsection{\label{subsec:qfreezing}Charge Freezing}
%
\begin{wrapfigure}[20]{r}{0.46\textwidth}
  \vspace{-\baselineskip}
  \vspace{-\baselineskip}
  \centering
...
%
%In this work, we restrict our attention to a 2D \(U(1)\) lattice gauge theory
%and compare our approach with that of traditional HMC.
%
%It is worth mentioning that, while there exist classical alternatives which
%outperform HMC at sampling the topology, (see e.g.~\cite{Eichhorn:2021ccz} and
%references therein for a comparison), they are of limited practical use as they
%cannot be straightforwardly applied to full QCD.\footnote{In contrast to the
%proposed method which, at least in principle, is generally applicable.}

% they are restricted to simplified models whereas our approach
% can (in principle) be straightforwardly applied to full QCD.

% cannot be
% straightforwardly applied to full QCD .
% models and cannot be straightforwardly applied to full QCD.

% are known to outperform
% HMC they are restricted to simplified models and cannot be straightforwardly
% applied to full QCD.
%
% the 2D \(U(1)\) model, their
% scope is severly limited and cannot be straightforwardly applied to full QCD.
% simplified models and edge cases whereas our approach, in principle, should
% apply straightforwardly to full QCD.

% they are applicable only to this toy model and the problem for full QCD
% remains.

% While there exist \emph{beter} methods than generic HMC (se,
% e.g.~\cite{Eichhorn:2021ccz} for a comparison of different approaches), 

% The problem of topological gauge configuration has model been studied (see,
% e.g.~\cite{Eichhorn:2021ccz} for a comparison of different approaches) with varying success for the 2D U(1) model
% implementation whose behavior is well understood\footnote{While \emph sa.
%   sampling methods exist, our uiltimate goal is eventually .
%

% In this work, we consider a \(U(1)\) gauge model on a 2D periodic lattice.
% % %
% % This simplified gauge theory serves as simple toy-model that
% % exhibits qualitative behavior similar to that of full QCD while remaining
...
% }.
% %
% % ~\cite{Duerr2012} classical methods which mix topological mixing 
% % with that eliminate charge freezing 
% % and classical methods for topological mixing .
% %
...
% while remaining computationally feasible and easy to work with.%~\footnote{allows
% % for rapid prototyping / testing new ideas}.
% %
% % We are interested in this model due to its relatiuve simplicity, as well as
% % its qualitative behavior which 00

% We are interested in this model primarily due to its relative simplicity
